"""Intent parsing node for NL2SQL graph - LLM-based only."""
...
class QueryIntentSchema(BaseModel):
    ...
    metrics: List[str] = Field(
        description="要查询的指标字段列表，如 ['gmv', 'order_count']"
    )
...
INTENT_PROMPT = """你是数据分析意图解析专家，负责将用户的自然语言查询转换为结构化的分析意图。

## 任务
从用户查询中提取以下要素：
1. **metrics**: 要查询的指标字段（如 gmv, order_count, user_count, paid_amount）
2. **dimensions**: 分组维度（如 date, province, category, user_level）
3. **time_range**: 时间范围，支持自然语言（如 昨天, 近7天, 2024-12-21, 本月）
4. **filters**: 过滤条件（如 province='北京', user_level=2）
...
6. **aggregation**: 聚合方式（sum/count/avg/max/min，默认sum）
